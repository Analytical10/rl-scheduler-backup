...
from para_eff_pt.pt_LoHi.LoHi_v1 import *
from para_eff_pt.pt_LoHi.LoHi_v101 import *
from para_eff_pt.pt_LoHi.LoHi_v201 import *

from para_eff_pt.utils.train_utils import *
from para_eff_pt.utils.args import parse_args

# Import RL Wrapper for isinstance check
from para_eff_pt.pt_rl_opt.rl_optimizer import RL_AdamW_Wrapper
from para_eff_pt.pt_rl_opt.r_adam import R_AdamW
...
def main(args):
    ...
    # Pass local_rank to args for optimizer builder
    ...
    is_rl_optimizer = isinstance(optimizer, (RL_AdamW_Wrapper,R_AdamW))
    if is_rl_optimizer:
        logger.info("RL/R_AdamW Optimizer detected: Standard scheduler stepping will be DISABLED.")
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if not layer_wise_flag:
            if is_rl_optimizer:
                # 1. 克隆 loss 避免影响反向传播图
                rl_loss = loss.detach().clone()
                
                # 2. 执行 AllReduce (平均)
                dist.all_reduce(rl_loss, op=dist.ReduceOp.AVG)
                
                # 3. 使用同步后的 loss 传给 optimizer
                # [MODIFIED] Capture abort signal from RL Optimizer
                _, abort_training = optimizer.step(loss=rl_loss)
                
                if abort_training:
                    if global_rank == 0:
                        logger.error(f"!!! ABORTING TRAINING DUE TO LOSS SPIKE at Step {update_step} !!!")

                        # 确保目录存在
                        os.makedirs(args.save_dir, exist_ok=True)

                        crash_agent_path = os.path.join(args.save_dir, f"agent_CRASH_recovery_step{update_step}.pth")
                        
                        optimizer.save_agent(os.path.join(args.save_dir, "agent_latest_crash.pth")) 

                        logger.warning(f"⚠️  Emergency Saving RL Agent to: {crash_agent_path}")
                        logger.warning("This agent has received a penalty for the crash and should be used for recovery.")
                        
                        
                        
                        # 调用 wrapper 的保存方法
                        if hasattr(optimizer, "save_agent"):
                            optimizer.save_agent(crash_agent_path)
                        else:
                            # 如果 wrapper 没有 save_agent 方法的备用方案
                            torch.save(optimizer.agent.state_dict(), crash_agent_path)
                    # Force exit to notify Bash script
            ...
            elif isinstance(optimizer, RL_AdamW_Wrapper):
                # RL Optimizer needs loss to compute state features
                # [MODIFIED] Capture abort signal
                _, abort_training = optimizer.step(loss=loss)
                if abort_training:
                    if global_rank == 0:
                        logger.error(f"!!! ABORTING TRAINING DUE TO LOSS SPIKE at Step {update_step} !!!")
                    sys.exit(1)
        ...
        if (
            ...
            if is_rl_optimizer:
                # 构造文件名: agent_epoch{N}_step{M}.pth
                agent_filename = f"agent_epoch{args.rl_epoch}_step{update_step}.pth"
                agent_save_path = os.path.join(args.rl_agent_save_dir, agent_filename)
                
                logger.info(f"Saving RL Agent to {agent_save_path}")
                os.makedirs(args.rl_agent_save_dir, exist_ok=True)
                
                # 调用 wrapper 的保存方法 (假设你在 wrapper 里实现了 save_agent)
                # 如果 wrapper 没有 save_agent，可以使用 optimizer.agent.save(path)
                optimizer.save_agent(agent_save_path)
                
                # 同时保存一个 "latest" 指针，方便下一轮 epoch 读取
                latest_path = os.path.join(args.rl_agent_save_dir, f"agent_epoch{args.rl_epoch}_final.pth")
                optimizer.save_agent(latest_path)
        ...
        if update_step % args.cycle_length == 0 and (args.peft_model.lower() == 'lohi_v1') and args.restart:
            logger.info(f"\nReinitialize B,A at update step {update_step}")
            
                        
            underlying_model = model.module if hasattr(model, 'module') else model
            for name, module in underlying_model.named_modules():   
                if isinstance(module, QuantPretrainLinear):
                    module.merge_and_reinit()
        ...
        if not layer_wise_flag:
            ...
            ## average lr 是所有 param group 的 lr 平均值
            average_lr = sum([group["lr"] for group in optimizer.param_groups]) / len(optimizer.param_groups)
        else:
            lr = list(optimizer.values())[0].param_groups[0]["lr"]
            current_wd = list(optimizer.values())[0].param_groups[0].get("weight_decay", 0.0)
            average_lr = sum([group["lr"] for opt in optimizer.values() for group in opt.param_groups]) / sum([len(opt.param_groups) for opt in optimizer.values()])
        ...
        if global_rank == 0:
            wandb.log(
                {
                    "loss": loss.item(),
                    "lr": lr,
                    "weight_decay": current_wd, # <--- 新增: 记录 Weight Decay --->
                    "update_step": update_step,
                    "tokens_seen": tokens_seen,
                    ...
                    # "paras_norm": paras_norm,
                    
                },
                # step=global_step,
                step = update_step,
            )
            if "gam" in args.optimizer.lower():
                wandb.log(
                    {
                        'agreement_ema': optimizer.agreement_ema,
                    },
                    step = update_step,
                )
            ...
            if "rl" in args.optimizer.lower() or "r_" in args.optimizer.lower():
                lrs = [g["lr"] for g in optimizer.param_groups]
                wandb.log(
                    {
                        "lr_shallow": lrs[0] if len(lrs) > 0 else 0,
                        "lr_middle":  lrs[1] if len(lrs) > 1 else 0,
                        "lr_deep":    lrs[2] if len(lrs) > 2 else 0,
                    },
                    step = update_step,
                )
