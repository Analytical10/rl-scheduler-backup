...
from para_eff_pt.pt_2nd_opt import UnbalancedMomentum
 
from para_eff_pt.pt_LoHi import LoRa_pretrain_Model

from para_eff_pt.pt_LoHi import QuantPretrainModel, QuantPretrainConfig

from para_eff_pt.pt_LoHi.LoHi_v101 import QuantPretrainConfig_v101, QuantPretrainModel_v101
from para_eff_pt.pt_LoHi.LoHi_v201 import QuantPretrainConfig_v201, QuantPretrainModel_v201
from para_eff_pt.pt_LoHi.LoHi_v202 import QuantPretrainConfig_v202, QuantPretrainModel_v202

from para_eff_pt.pt_GAM import SA_GAM_AdamW
from para_eff_pt.pt_GAM import AdEMAMix
from para_eff_pt.pt_rl_opt import *
...
def build_model(model, args):
    if args.peft_model.lower() == "low-rank":
    ...
    elif args.peft_model.lower() == "lohi_v1":
    
        # 1. 创建 QuantPretrainConfig 配置对象
        #    这个对象会收集所有与我们模型相关的超参数
        config = QuantPretrainConfig(
            r=args.rank,
            lora_alpha=args.lora_alpha,
            lora_dropout=args.lora_dropout,
            target_modules=args.target_modules,
            quant_format=args.quant_format,  # 'nf4' 或 'mxfp4'
        )
        
        # 2. 使用配置对象和基础模型来实例化我们的 QuantPretrainModel
        model = QuantPretrainModel(model, config)
    
    elif args.peft_model.lower() == "lohi_v101":
        config = QuantPretrainConfig_v101(
            r=args.rank,
            lora_alpha=args.lora_alpha,
            lora_dropout=args.lora_dropout,
            target_modules=args.target_modules,
        ...
        model = QuantPretrainModel_v101(model, config)
        
    elif args.peft_model.lower() == "lohi_v201":
        print(f"Building model with LoHi_v201 using NVFP4 simulation...")
        config = QuantPretrainConfig_v201(
            r=args.rank,
            lora_alpha=args.lora_alpha,
            lora_dropout=args.lora_dropout,
            target_modules=args.target_modules,
            quant_bits=4, # 固定为4-bit
            quant_block_size=16, # 固定为16，符合NVFP4
        )
        model = QuantPretrainModel_v201(model, config)
        
    elif args.peft_model.lower() == "lohi_v202":
        print(f"Building model with LoHi_v202 (DGE + OCC-LoRA)...")
        config = QuantPretrainConfig_v202(
            r=args.rank,
            lora_alpha=args.lora_alpha,
            lora_dropout=args.lora_dropout,
            target_modules=args.target_modules,
            quant_block_size=16, # 固定为16，符合NVFP4
            occ_quantile=args.occ_quantile,
            dge_k=args.dge_k,
        )
        model = QuantPretrainModel_v202(model, config)
...
def build_optimizer(model, trainable_params, args):   
    ...
    elif args.optimizer.lower() == "gam":
        
        optimizer = SA_GAM_AdamW(
            trainable_params, lr=args.lr, weight_decay=args.weight_decay,
        )
        
    elif args.optimizer.lower() == "ademamix":
        
        beta3_warmup = alpha_warmup = args.num_training_steps
        
        optimizer = AdEMAMix(
            model.parameters(),
            lr=args.lr,
            betas=(0.9, 0.999, 0.999), # (beta1, beta2, beta3_final)
            alpha=8.0,                   # 论文中常用的alpha值在[5, 10]之间
    ...
    elif args.optimizer.lower() == "unbalanced_momentum_v1":
        thetas = (args.theta1, args.theta2)
        powers = (args.pow_diag, args.power)
    ...
    # ==========================================
        ...
        print(f"Initializing RL_AdamW_Wrapper with max_lr={args.lr}, min_lr={args.lr * args.min_lr_ratio}")
        
        # Ensure local_rank is available (usually passed via args or env)
        local_rank = getattr(args, 'local_rank', 0)
        device = f"cuda:{local_rank}"
        
        optimizer = RL_AdamW_Wrapper(
            trainable_params,
            lr=args.lr, # Base LR (acts as max_lr for cosine schedule inside wrapper)
            weight_decay=args.weight_decay,
            # --- RL Wrapper Specific Args ---
            max_lr=args.lr,
            ...
            mode=args.rl_mode, # Default to train
    ...
    elif args.optimizer.lower() == "r_adamw":
        print(f"Initializing R_AdamW_Noise with max_lr={args.lr}, min_lr={args.lr * args.min_lr_ratio}")
        # Ensure local_rank is available (usually passed via args or env)
        local_rank = getattr(args, 'local_rank', 0)
        device = f"cuda:{local_rank}"
        optimizer = R_AdamW(
