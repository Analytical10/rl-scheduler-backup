...
def parse_args(args):
    ...
    parser.add_argument("--save_every", type=int, default=10_0000)
    ...
    ## LoHi_v1 parameters
    parser.add_argument("--quant_format", type=str, default="nf4", choices=["nf4", "mxfp4"])
    parser.add_argument("--restart", action="store_true", default=False)

    ## LoHi_v201 parameters
    parser.add_argument(
        "--qaf_start_step",
        type=int,
        default=-1,
        help="The update step to start Quantization-Aware Finetuning (QAF). -1 to disable.",
    )
    
    ## LoHi_v202 parameters
    parser.add_argument(
        "--occ_quantile",
        type=float,
        default=0.999,
        help="Quantile for outlier clamping in OCC mechanism (for lohi_v202).",
    )
    parser.add_argument(
        "--dge_k",
        type=float,
        default=5.0,
        help="Hyperparameter k for Differentiable Gradient Estimator (for lohi_v202).",
    )
    
    ## UNM
    parser.add_argument("--subspace_ratio", type=float, default=0.01)
    parser.add_argument("--theta1", type=float,  default=0.9)
    parser.add_argument("--theta2", type=float,  default=0.9)
    parser.add_argument("--power", type=float, default=-0.5)
    parser.add_argument("--pow_diag", type=float, default=-0.5)
    parser.add_argument("--ire", type=float, default=1.0)
    ...
    # ==========================================
    parser.add_argument("--rl_agent_lr", type=float, default=3e-4, help="Learning rate for the PPO Agent")
    parser.add_argument("--rl_history_len", type=int, default=5, help="History window size for RL state")
    parser.add_argument("--rl_stats_window", type=int, default=20, help="Window size for calculating loss std/trend")
    parser.add_argument("--rl_agent_save_dir", type=str, default="rl_checkpoints", help="Directory to save RL agent checkpoints")
    parser.add_argument("--rl_agent_load_path", type=str, default=None, help="Path to load a pre-trained RL agent (.pth file)")
    parser.add_argument("--rl_epoch", type=int, default=1, help="Current meta-training epoch index (for saving naming)")
    parser.add_argument("--rl_mode", type=str, default="train", choices=["train", "eval"], help="Mode for RL agent")
    ...
    ## R_adamw
    parser.add_argument("--r_noise_std", type=float, default=0.0)
    parser.add_argument("--r_noise_mean", type=float, default=0.0)
